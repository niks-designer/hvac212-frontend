...
function buildTwitter(seo?: PageSeoData): Metadata["twitter"] | undefined {
  ...
  const description = seo.twitter_description || seo.og_description || seo.description;
...
function buildMetadata(page: WordPressPageSeoResponse): Metadata {
  const seo = page.seo;
  const title = seo?.title || page.title;
  const description = seo?.description || page.excerpt || "";
...
export async function getPageSEO(slug: string): Promise<Metadata> {
  try {
    const page = await wpGet<WordPressPageSeoResponse>(`hvac/v1/page/${slug}`);
